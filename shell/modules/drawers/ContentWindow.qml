...
StyledWindow {
    ...
    // matte skin: blob fill killed — every panel now has its own floating matte
    // Bubble island, so the connected SDF blob (frame + panel tails) draws nothing
    property color surfaceColour: "transparent"
    ...
    Item {
        ...
        BlobInvertedRect {
            ...
            // matte skin: no connecting frame — the bar is a discrete floating
            // island (see BarWrapper), so the blob no longer draws the left strip
