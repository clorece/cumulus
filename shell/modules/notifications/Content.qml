...
Item {
    ...
    component NotifWrapper: Item {
        ...
        // toast AO — outside the clip below (which would strangle the
        // bubble's own shadow); fades out as the toast slides to dismiss
        Elevation {
            anchors.fill: clipRect
            radius: notif.radius
            level: 2
            z: -1
            opacity: 1 - Math.min(1, Math.abs(notif.x) / Math.max(1, notif.implicitWidth))
        }
