...
Item {
    ...
    // height only reserves the top margin: bottom-anchored to the panel
    // area (10px off the screen edge), so the bottom card floats at the
    // same island offset as every other surface
    readonly property real nonAnimHeight: ((content.item as Content)?.nonAnimHeight ?? 0) + content.anchors.margins
    property real offsetScale: shouldBeActive ? 0 : 1
    property real sidebarLerp

    visible: offsetScale < 1
    anchors.bottomMargin: (-implicitHeight - 5) * offsetScale
    implicitHeight: content.implicitHeight + content.anchors.margins
